...
int main(int argc, char** argv) {
    try {
        ...
        const int compute_iters = args.has("compute-iters")
                                      ? args.get_int("compute-iters", 64)
                                      : args.get_int("fmas-per-load", 64);
        ...
        const std::size_t thread_count =
            static_cast<std::size_t>(launch_grid) * static_cast<std::size_t>(block);
        const std::size_t bytes_out =
            thread_count * sizeof(float);
        ...
        init_array_kernel<<<launch_grid, block>>>(d_out, 0.0f, static_cast<int>(thread_count));
        CUDA_CHECK_LAST_KERNEL();
        CUDA_CHECK(cudaDeviceSynchronize());

        auto launch = [&]() {
            mem_compute_mix_kernel<<<launch_grid, block>>>(d_in, d_out, n, compute_iters);
            CUDA_CHECK_LAST_KERNEL();
        };
        ...
        //   - stream reads over n input elements
        //   - one final store per launched thread
        //   - each processed element performs compute_iters FMAs => 2 FLOPs each
        //
        // IMPORTANT:
        //   - bytes_effective is a MODEL quantity, not a measured HW traffic counter.
        //   - ai_nominal is a MODEL arithmetic intensity, not a measured DRAM intensity.
        const double flops_total =
            static_cast<double>(n) * 2.0 * static_cast<double>(compute_iters);
        ...
        if (!csv_path.empty()) {
            ...
            writer.write_row({
                ...
                bw::to_csv_string(n),
                bw::to_csv_string(bytes_in),
                bw::to_csv_string(bytes_out),
                bw::to_csv_string(bytes_effective),
                bw::to_csv_string(thread_count),
                bw::to_csv_string(block),
                bw::to_csv_string(launch_grid),
                bw::to_csv_string(compute_iters),
